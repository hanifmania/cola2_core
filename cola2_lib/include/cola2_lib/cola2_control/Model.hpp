...
public:
    ...
    std::vector< double >
    compute( const std::vector< double > twist_feedback,
             const std::vector< double > pose_feedback )
    {
        ...
        float X_model = Mw*w*q-Mv*v*r+(W-B)*sin(_theta)-Xu*u-Xuu*u*abs(u);
        ret.push_back(X_model);
        float Y_model = -Mw*w*p+Mu*u*r-(W-B)*cos(_theta)*sin(_phi)-Yv*v-Yvv*v*abs(v);
        ret.push_back(Y_model);
        float Z_model = Mv*v*p-Mu*u*q-(W-B)*cos(_theta)*cos(_phi)-Zw*w-Zww*w*abs(w);
        ret.push_back(Z_model);
        ret.push_back(0.0);
        float M_model = -zB*B*sin(_theta);
        ret.push_back(M_model);
        float N_model = Mv*v*u-Mu*u*v+Mq*q*p-Mp*p*q-Nr*r-Nrr*r*abs(r);
