...
public:
    ...
    std::vector< double >
    compute( const std::vector< double > feedback,
             Request setpoint )
    {
        ...
        if( !setpoint.getDisabledAxis().at(5) ) ret.push_back(N);
        else ret.push_back(0.0);
