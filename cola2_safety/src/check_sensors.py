    ...
    def check_timers(self, event):
        ...
        if self.check_bumblebee:
            ...
            if now - self.last_bumblebee_update > 2.0:
                self.diagnostic.setLevel(DiagnosticStatus.WARN, "No Bumblebee images!")
                is_ok = False
        ...
        if self.check_profiler:
            ...
            if now - self.last_profiler_update > 2.0:
                self.diagnostic.setLevel(DiagnosticStatus.WARN, "No Profiler data!")
                is_ok = False
        ...
        if self.check_micron:
           ...
           if now - self.last_micron_update > 2.0:
                self.diagnostic.setLevel(DiagnosticStatus.WARN, "No Micron data!")
                is_ok = False
